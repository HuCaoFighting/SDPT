...
class Block(BaseModule):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(
            ...
            attn_drop=attn_drop, proj_drop=drop, pool=pool)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=nn.Hardswish, drop=drop, ksize=3)
...
class ConvStem(BaseModule):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        self.proj = torch.nn.Sequential(
                ...
                nn.GELU(),
                conv3x3(embed_dim // 2, embed_dim // 2, 1),
                nn.GELU(),
                conv3x3(embed_dim // 2, embed_dim // 2, 2),
                nn.GELU(),
                conv3x3(embed_dim // 2, embed_dim, 1),
            )
...
class SDPTVisionTransformer(BaseModule):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 4, 6, 3], pool=[True, True, True, False], pretrained=None,
                 init_cfg=None):
        super(SDPTVisionTransformer, self).__init__(init_cfg=init_cfg)
        ...
        self.patch_embed1 = ConvStem(img_size=img_size, patch_size=4, stride=4, in_chans=in_chans,
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            ...
            super(SDPTVisionTransformer, self).init_weights()
